#' Peudo regression coefficients from hybrid generalized partial 
#' correlation coefficients (HGPCC).
#'
#' This function gets HGPCCs by calling  \code{parcorVecH} function.
...
#'
#' @note Hybrid Generalized Partial Correlation Coefficients (HGPCC) allow comparison of
...
hgpcc=parcorVecH(mtx=mtx,ctrl=ctrl, verbo=verbo, idep=idep)
if (verbo) print(hgpcc)
nam1=colnames(hgpcc)
if(verbo) print(nam1)
pp=length(hgpcc)
if(verbo) print(pp)
out=rep(NA,pp)
for (i in 1:pp){
out[i]=hgpcc[i]*mysd[idep]/sd(mtx[,nam1[i]],na.rm = TRUE)
}#end for loop
  names(out) = nam1
  return(out)
}
