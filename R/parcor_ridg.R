...
#'  increas the \code{incr} till all partial correlations are within the [-1,1] interval.}
...
#' mtx=cbind(x,y,z)
#' g1=gmcmtx0(mtx)
#' parcor_ijk(g1,1,2) # ouji> ouij implies i=x is the cause of j=y
...
parcor_ridg <- function(gmc0, dig = 4, idep = 1, verbo = FALSE, incr = 3) {
    # input is asymmetric generalized correlation matrix dig=4 #digits of accuracy
    # desired We want r* of idep, with respect to others removing all others as k
    n = NROW(gmc0)
    p = NCOL(gmc0)
    if (n != p) 
        stop("gmc0 is not square matrix")
    ...
    # now we ridge adjust the generalized correlation matrix gmc0 by adding ridgek
    ...
    for (jridge in 1:10) {
        ...
        # if ( (max(abs(partij))>1) | (max(abs(partji))>1)){ print(c('iteration for
        # ridgek=',ridgek, jridge))}
